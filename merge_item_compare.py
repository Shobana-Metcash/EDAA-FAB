#!/usr/bin/env python3
"""
Script to merge CDL and GITHUB tabs from item_compare.xlsx

Matching Rules:
- CDL Column H (Table Field Name) matches GITHUB Column D (cdm_column), OR
- CDL Column L (Biz Name) matches GITHUB Column E (pdm_column)

Output:
- A sheet named "item_compare_merged" with:
...
def get_default_output_path(input_file):
    ...
    Returns:
        str: Path to results/item_compare_results.xlsx
    """
    results_dir = os.path.join(os.path.dirname(os.path.abspath(input_file)), 'results')
    os.makedirs(results_dir, exist_ok=True)
    return os.path.join(results_dir, 'item_compare_results.xlsx')

def merge_item_compare(input_file='item_compare.xlsx', output_file=None):
    """
    Merge CDL and GITHUB sheets based on matching criteria.
    
    Args:
        input_file: Path to the input Excel file (default: item_compare.xlsx)
        output_file: Path to the output Excel file (default: results/item_compare_results.xlsx)
                    If None, saves to results/item_compare_results.xlsx
    """
    try:
        ...
        cdl_col_h = 'Table Field Name'  # Column H (index 7)
        cdl_col_l = 'Biz Name'  # Column L (index 11)
        github_col_d = 'cdm_column'  # Column D (index 3)
        github_col_e = 'pdm_column'  # Column E (index 4)
        
        # Validate that required columns exist
        if cdl_col_h not in cdl_df.columns:
            raise ValueError(f"CDL sheet must contain column '{cdl_col_h}'")
        if cdl_col_l not in cdl_df.columns:
            raise ValueError(f"CDL sheet must contain column '{cdl_col_l}'")
        ...
        for cdl_idx, cdl_row in cdl_df.iterrows():
            cdl_value_h = cdl_row[cdl_col_h]
            cdl_value_l = cdl_row[cdl_col_l]
            ...
            for github_idx, github_row in github_df.iterrows():
                ...
                # Check if CDL Column H matches GITHUB Column D
                if values_match(cdl_value_h, github_value_d):
                    if github_idx in matched_github_indices:
                        # This GITHUB record was already matched to another CDL record
                        is_duplicate = True
                        match_type = 'Duplicated (CDL Column H matches GITHUB Column D - already matched)'
                        break
                    matched_github_row = github_row
                    matched_github_idx = github_idx
                    match_type = 'CDL Column H matches GITHUB Column D'
                    break
                
                # Check if CDL Column L matches GITHUB Column E
                if values_match(cdl_value_l, github_value_e):
                    if github_idx in matched_github_indices:
                        # This GITHUB record was already matched to another CDL record
                        is_duplicate = True
                        match_type = 'Duplicated (CDL Column L matches GITHUB Column E - already matched)'
                        break
                    matched_github_row = github_row
                    matched_github_idx = github_idx
                    match_type = 'CDL Column L matches GITHUB Column E'
                    break
        ...
        # Save to Excel file with sheet name "item_compare_merged"
        print(f"\nSaving to {output_file}...")
        merged_df.to_excel(output_file, index=False, sheet_name='item_compare_merged')
...
if __name__ == "__main__":
    ...
    input_file = sys.argv[1] if len(sys.argv) > 1 else 'item_compare.xlsx'
    
    # Set default output file to results/item_compare_results.xlsx
    if len(sys.argv) > 2:
    ...
    print("="*80)
    print("CDL and GITHUB Sheet Merge Tool for item_compare.xlsx")
    print("="*80)
    print(f"Input file: {input_file}")
    print(f"Output file: {output_file}")
    print()
    
    result = merge_item_compare(input_file, output_file)
